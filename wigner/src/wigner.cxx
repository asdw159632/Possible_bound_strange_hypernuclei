...
int main(int argc, char *argv[])
{
	...
	TFile *read=new TFile(filepath,"update");
	...
	TH3D wigdst("wigdst","wigner density plot",Nr,rmin,rmax,Np,pmin,pmax,Ntheta,thetamin,thetamax);
	...
	char savepath[100];
	sprintf(savepath,"./wigdst/%s.root",file);
	TFile save(savepath,"create");
	if(save.IsZombie())
	{
		cerr<<"file "<<savepath<<" already exist"<<endl;
		return 1;
	}
	wigdst.Write();
	save.Close();
