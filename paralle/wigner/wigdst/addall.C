void addall()
{
	...
	if(scale==1)
	{
		sprintf(readpath0,"%s/%s_thetal_1-scaled.root",workdir,workdir);
	}else{
		sprintf(readpath0,"%s/%s_thetal_1.root",workdir,workdir);
	}
	TFile read0(readpath0,"read");
	TH2D *readth0=(TH2D*)read0.Get("wigdst_thetal_1");
