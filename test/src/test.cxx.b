...
{
	double r=x[1];//hyper spherical radius
	double alpha=x[0];//hyper spherical angle
...
{
  TF2 *test=new TF2("test",integral_H,0,TMath::Pi()/2,0,50,2);
  double n=1;
  double m=1;
  l=0;
  c=10;
  if(argc==3){n=atof(argv[1]);m=atof(argv[2]);}
  if(argc==4){n=atof(argv[1]);m=atof(argv[2]);c=atof(argv[3]);}
  test->SetParameters(n,m);
  cout<<"n="<<n<<", m="<<m<<", integral result:"<<test->TF2::Integral(0.,TMath::Pi()/2,0.,50.,1e-50)<<endl;
  double hx[]={Pi/4,1};
  double hpar[]={n,m};
  cout<<integral_H(hx,hpar)<<endl;
  delete test;
}
